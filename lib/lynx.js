...
Lynx.prototype.send = function send(stats, sample_rate) {
  ...
  if(all_stats.length === 0) {
    if (Object.keys(stats).length > 0) {
      //
      // There were stats that were sampled away
      //
      return;
    }
    //
...
Lynx.prototype.send_data = function send_data(send_data) {
  //
  // Encode our data to a buffer
  //
  var self          = this
    , buffer = new Buffer(send_data, 'utf8')
    , socket
    ;
...
Lynx.prototype._update_last_used = function _update_last_used() {
  ...
  if (this.ephemeral_socket) {
    ...
    //
    // If the timer supports unref, allow application to close
    //
    if (self.last_used_timer.unref) {
      self.last_used_timer.unref();
    }
  }
};
